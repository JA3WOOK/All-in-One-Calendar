...
function App() {
  ...
  const fetchSchedules = (teamList = []) => {
    API.get("/api/schedules")
      ...
      .then((data) => {
        const formatted = data.map((item) => {
          ...
          return {
            ...
            ...(item.team_id
              ? (() => {
                  const t = teamList.find(
                    (t) => String(t.team_id) === String(item.team_id),
                  );
      ...
      .catch((err) =>
        console.error("일정 로딩 에러:", err?.response?.data || err.message),
      );
  };
  ...
  const fetchTeams = () => {
    API.get("/api/teams")
      ...
      .then((data) => {
        ...
        data.forEach((team) => {
          initialTeamFilters[String(team.team_id)] = true;
        });
      ...
      const teamResults = await Promise.all(
        teamList.map((team) =>
          API.get("/api/todos/calendar", {
            params: { year, month, team_id: team.team_id },
          })
            .then((r) => ({
  ...
  const renderMiniDays = () => {
    ...
    for (let d = 1; d <= lastDay; d++) {
      ...
      const dayEvents = dbEvents.filter(
        (e) => e.start && e.start.startsWith(currentStr),
      );
      const hasPersonal = dayEvents.some((e) => !e.extendedProps?.team_id);
      const hasTeam = dayEvents.some(
        (e) => e.extendedProps?.team_id && !e.extendedProps?.is_study,
      );
      ...
      days.push(
        <div
          ...
          className="mini-day"
          onClick={() =>
            calendarRef.current.getApi().gotoDate(new Date(year, month, d))
          }
        >
  ...
  const handleEventClick = (info) => {
    ...
    if (ep.eventType === "todo") {
      ...
      const todoTeamRole = ep.team_id
        ? (teams.find((t) => String(t.team_id) === String(ep.team_id))?.role ??
          "VIEWER")
        : null;
      setSelectedEvent({
        type: "todo",
        ...
        repeatEndAt: ep.repeat_end_at
          ? String(ep.repeat_end_at).slice(0, 10)
          : "",
      ...
      setSelectedEvent({
        ...
        teamId: ep.team_id ?? null,
        viewOnly: ep.team_id
          ? (teams.find((t) => String(t.team_id) === String(ep.team_id))
              ?.role ?? "VIEWER") === "VIEWER"
          : false,
      });
    }
  ...
  const handleCreateSubmit = async (data) => {
    try {
      if (data.type === "schedule") {
        ...
        const locationObj =
          rawLoc && rawLoc.address
            ? {
                name: rawLoc.name || rawLoc.address,
  ...
  const handleSaveEditTeam = () => {
    ...
    API.put(`/api/teams/${editingTeam.team_id}`, {
  ...
  const filteredSchedules = dbEvents.filter((event) => {
    if (event.id && String(event.id).startsWith("holiday"))
  ...
  let personalCount = 0;
    ...
    // 공휴일(holiday)은 통계에서 제외하고 실제 유저 일정/할일만 필터링
    const realEvents = filteredEvents.filter(e => !(e.id && String(e.id).startsWith('holiday')));

    realEvents.forEach(evt => {
        const ep = evt.extendedProps || {};
        const isTeam = !!ep.team_id; // team_id가 있으면 그룹, 없으면 개인
        
        if (isTeam) groupCount++;
        else personalCount++;

        if (ep.eventType === 'todo') {
            // 할일(Todo)인 경우
            todoTotal++;
            if (ep.isDone || ep.is_done) todoCompleted++;
        } else {
            // 일정(Schedule)인 경우
            scheduleTotal++;
            if (ep.isDone || ep.is_done) scheduleCompleted++;
        }
    });

    const totalCount = personalCount + groupCount;
    // 0으로 나누는 에러 방지 (등록된 게 없으면 0%)
  ...
  const renderEventContent = (eventInfo) => {
    const { eventType, isDone, priority, team_color } =
  ...
  return (
        ...
        <div className="mini-calendar-card">
          ...
          <div className="mini-grid">
            {["S", "M", "T", "W", "T", "F", "S"].map((d, i) => (
              <div key={`${d}-${i}`} className="mini-day-label">
                {d}
              </div>
            ))}
            {renderMiniDays()}
                    ...
                    onClick={() =>
                      setFilters((prev) => ({
                        ...
                        teams: {
                          ...prev.teams,
                          [String(team.team_id)]:
                            !prev.teams[String(team.team_id)],
                        },
                      }))
                    }
                  >
          ...
          {isPersonalOpen && (
            <div className="category-list">
              ...
              <div
                ...
                onClick={() =>
                  setFilters((prev) => ({ ...prev, personal: !prev.personal }))
                }
              >
                <span
                  style={{
                ...
                <span
                  style={{
                    color: filters.personal ? "#2d2d2d" : "#9ca3af",
                    transition: "color 0.15s",
                  }}
                >
                  개인 일정
              ...
              <div
                ...
                onClick={() =>
                  setFilters((prev) => ({
                    ...prev,
                    personalTodo: !prev.personalTodo,
                  }))
                }
              >
                <span
                ...
                <span
                  style={{
                    color: filters.personalTodo ? "#2d2d2d" : "#9ca3af",
                    transition: "color 0.15s",
                  }}
                >
                  개인 할일
    ...
    {isStatsOpen && (
        ...
        >
            <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '10px', color: '#4b5563', fontSize: '12px' }}>
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"></rect><line x1="16" y1="2" x2="16" y2="6"></line><line x1="8" y1="2" x2="8" y2="6"></line><line x1="3" y1="10" x2="21" y2="10"></line></svg>
                <span>전체 일정</span>
            </div>
            
            {/* 전체 갯수 동적 연동 */}
            <div style={{ fontSize: '24px', fontWeight: 'bold', color: '#111827', marginBottom: '2px' }}>
                {totalCount}개
            </div>
            {/* 개인/그룹 갯수 동적 연동 */}
            <div style={{ fontSize: '11px', color: '#6b7280', marginBottom: '20px' }}>
                개인: {personalCount} 그룹: {groupCount}
            </div>

            {/* 일정 완료율 (Schedule) */}
            <div style={{ marginBottom: '16px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '11px', marginBottom: '6px' }}>
                    <span style={{ color: '#4b5563' }}>일정 완료율</span>
                </div>
                <div style={{ fontSize: '15px', fontWeight: 'bold', marginBottom: '6px', color: '#111827' }}>{scheduleRate}%</div>
                <div style={{ width: '100%', background: '#e5e7eb', height: '6px', borderRadius: '4px', overflow: 'hidden' }}>
                    {/* 너비(width)를 %에 맞게 동적 연동 */}
                    <div style={{ background: '#111827', height: '100%', width: `${scheduleRate}%`, borderRadius: '4px', transition: 'width 0.4s ease' }} />
                </div>
            </div>
            ...
            <div>
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '11px', marginBottom: '6px' }}>
                    <span style={{ color: '#4b5563' }}>할일 완료율</span>
                </div>
                <div style={{ fontSize: '15px', fontWeight: 'bold', marginBottom: '6px', color: '#111827' }}>{todoRate}%</div>
                <div style={{ width: '100%', background: '#e5e7eb', height: '6px', borderRadius: '4px', overflow: 'hidden' }}>
                    {/* 너비(width)를 %에 맞게 동적 연동 */}
                    <div style={{ background: '#111827', height: '100%', width: `${todoRate}%`, borderRadius: '4px', transition: 'width 0.4s ease' }} />
                </div>
            </div>
        </div>
    )}
      ...
      {isGroupModalOpen && (
        <div
          ...
          onClick={(e) =>
            e.target === e.currentTarget && setIsGroupModalOpen(false)
          }
        >
          <div
            style={{
              ...
              {TEAM_COLOR_PALETTE.map((color) => (
                <button
                  ...
                  style={{
                    ...
                    outlineOffset: 2,
                    transform:
                      newGroupColor === color ? "scale(1.15)" : "scale(1)",
            ...
            >
              ...
              <button
                ...
                style={{
                  ...
                  border: "none",
                  background:
                    newGroupColor.length === 7 ? newGroupColor : "#4a80c4",
                  color: "#fff",
                  fontSize: 14,
                  fontWeight: 600,
                  cursor: "pointer",
      ...
      {isEditTeamModalOpen && editingTeam && (
        <div
          ...
          onClick={(e) =>
            e.target === e.currentTarget && setIsEditTeamModalOpen(false)
          }
        >
          <div
            style={{
          ...
          >
              ...
              {TEAM_COLOR_PALETTE.map((color) => (
                <button
                  ...
                  style={{
                    ...
                    transition: "transform 0.1s",
                    transform:
                      editTeamColor === color ? "scale(1.15)" : "scale(1)",
                    boxShadow:
                      editTeamColor === color
                        ? `0 0 0 2px #fff, 0 0 0 4px ${color}`
                        : "none",
            ...
            >
              ...
              <input
                ...
                onChange={(e) => {
                  const v = e.target.value;
                  if (/^#[0-9a-fA-F]{0,6}$/.test(v)) setEditTeamColor(v);
                }}
                placeholder="#hex"
                maxLength={7}
                style={{
            ...
            >
              ...
              <button
                ...
                style={{
                  ...
                  border: "none",
                  background:
                    editTeamColor.length === 7 ? editTeamColor : "#4a80c4",
                  color: "#fff",
                  fontSize: 14,
                  fontWeight: 600,
                  cursor: "pointer",
      ...
      {isDeleteTeamModalOpen && deletingTeam && (
        <div
          ...
          onClick={(e) =>
            e.target === e.currentTarget && setIsDeleteTeamModalOpen(false)
          }
        >
          <div
            style={{
          ...
          >
            ...
            >
              <span style={{ fontSize: 16, fontWeight: 600, color: "#c94f4f" }}>
                🗑️ 그룹 삭제
              </span>
              <button
            ...
            >
              ...
              >
                ...
                <br />
                소속된{" "}
                <strong style={{ color: "#991b1b" }}>모든 일정·할일</strong>도
                함께 삭제됩니다.
                <br />이 작업은 되돌릴 수 없습니다.
              </p>
            </div>
