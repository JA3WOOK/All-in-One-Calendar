...
export const fetchTeamMembers = (team_id) =>
    API.get(`/todos/team/${team_id}/members`).then((res) => res.data.data);

// ── 개인 Todo 생성 ───────────────────────────────
export const createPersonalTodo = (data) =>
    API.post("/todos/personal", {
...
export const fetchPersonalTodos = (due_date = null) =>
    API.get("/todos/personal", { params: due_date ? { due_date } : {} })
        .then((res) => res.data.data);

// ── 팀 Todo 목록 ─────────────────────────────────
export const fetchTeamTodos = (team_id, due_date = null) =>
    API.get(`/todos/team/${team_id}`, { params: due_date ? { due_date } : {} })
        .then((res) => res.data.data);

// ── 달력 월간 조회 ───────────────────────────────
export const fetchCalendarTodos = (year, month, team_id = null) =>
    API.get("/todos/calendar", { params: { year, month, ...(team_id ? { team_id } : {}) } })
        .then((res) => res.data.data);
