...
function generateRepeatDates(startDate, repeatType, interval, endDate) {
    ...
    while (current <= end) {
        ...
        if      (repeatType === "daily")   current.setDate(current.getDate() + interval);
        else if (repeatType === "weekly")  current.setDate(current.getDate() + 7 * interval);
        else if (repeatType === "monthly") current.setMonth(current.getMonth() + interval);
        else break; // 무한 루프 방지
    }
    return dates;
}